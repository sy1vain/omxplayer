...
class OmxPlayer extends EventEmitter {
  ...
  _stopProcess(cb){
    ...
    return new Promise((resolve, reject)=>{
      ps.lookup({command: 'omxplayer.bin'}, (err, results)=>{
        return Promise.all(results.map((result)=>{
          if(result.ppid!=pid) return Promise.resolve();
          return new Promise((resolve, reject)=>{
            ps.kill(result.pid, resolve);
          });
        }) ).then(resolve);
      });
    }).then(()=>{
  ...
  _invokeDBus(member, iface, signature, body, cb){
    if(!this.isRunning){
      cb && cb('omxplayer not running');
      return Promise.reject('omxplayer not running');
    }
    return this._getDBus().then((dbus)=>{
      if(!dbus){
